...
export default function SettingsPanel({ options, onChange, disabled, selectedPreset, presetLabel, onPresetChange, customPresets, onCustomPresetSelect, onSavePreset, onRenamePreset, onDeletePreset, onExportPresets, onImportPresets }: SettingsPanelProps): JSX.Element {
  ...
  return (<section className="panel settings-panel"><h2>Rename + compression settings</h2><p className="hint">{presetText}</p><p className="pill">{presetLabel}</p><div className="settings-section"><h3>Basic</h3><div className="grid">
    ...
    <label>Output format<select disabled={disabled} value={options.outputFormat ?? 'webp'} onChange={(e) => onChange({ ...options, outputFormat: e.target.value as GuiOptions['outputFormat'] })}><option value="webp">WebP — recommended</option><option value="avif">AVIF — smaller, slower</option><option value="jpeg">JPEG — photos only, no transparency</option><option value="png">PNG — lossless/transparency-safe</option></select><small className="field-help">Choose the format FoxPix will create. WebP is recommended for most web assets.</small></label>
    ...
    <div className="actions token-chips"><button type="button" className="secondary" onClick={() => insert('{name}')}>Original name</button><button type="button" className="secondary" onClick={() => insert('{prefix}')}>Prefix</button><button type="button" className="secondary" onClick={() => insert('{index}')}>Index</button><button type="button" className="secondary" onClick={() => insert('{folder}')}>Folder</button><button type="button" className="secondary" onClick={() => insert('{custom}')}>Custom text</button><button type="button" className="secondary" onClick={() => insert('-')}>-</button><button type="button" className="secondary" onClick={() => insert('_')}>_</button><button type="button" className="secondary" onClick={() => onChange({ ...options, pattern: '' })}>Clear pattern</button></div>
    <div className="actions pattern-presets"><button type="button" className="secondary" onClick={() => onChange({ ...options, pattern: '{name}' })}>Web-safe names</button><button type="button" className="secondary" onClick={() => onChange({ ...options, pattern: '{prefix}-{index}' })}>Prefix + index</button><button type="button" className="secondary" onClick={() => onChange({ ...options, pattern: '{prefix}-{name}' })}>Prefix + name</button><button type="button" className="secondary" onClick={() => onChange({ ...options, pattern: '{folder}-{name}' })}>Folder + name</button><button type="button" className="secondary" onClick={() => onChange({ ...options, pattern: '{custom}-{index}' })}>Custom + index</button><button type="button" className="secondary" onClick={() => onChange({ ...options, pattern: '{name}-{index}' })}>Name + index</button></div>
    <p className="hint">FoxPix makes filenames web-safe automatically. Underscores are converted to hyphens.</p>
    <div className="example-box"><p className="hint">Example 1: My Cute Animal.png → {ex1}</p><p className="hint">Example 2: Blue Sticker Pack.png → {ex2}</p>
  ...
  {advancedOpen ? <div className="settings-section advanced"><h3>Advanced</h3><div className="grid">
