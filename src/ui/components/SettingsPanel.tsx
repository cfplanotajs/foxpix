...
type RenamePreset = 'webSafe' | 'prefixIndex' | 'prefixName' | 'folderName' | 'custom';
...
function applyPatternPreset(preset: RenamePreset, currentPattern: string): string {
  if (preset === 'custom') return currentPattern;
  if (preset === 'webSafe') return '{name}';
  if (preset === 'prefixIndex') return '{prefix}-{index}';
  if (preset === 'prefixName') return '{prefix}-{name}';
  return '{folder}-{name}';
}

function detectPreset(pattern: string): RenamePreset {
  if (pattern === '{name}') return 'webSafe';
  if (pattern === '{prefix}-{index}') return 'prefixIndex';
  if (pattern === '{prefix}-{name}') return 'prefixName';
  if (pattern === '{folder}-{name}') return 'folderName';
  return 'custom';
}
...
function exampleName(options: GuiOptions): string {
  ...
  const resolved = options.pattern
    .replaceAll('{name}', slugify(sample))
    .replaceAll('{prefix}', prefix)
    .replaceAll('{index}', '001')
    .replaceAll('{folder}', 'stickers')
...
export default function SettingsPanel({ options, onChange, disabled }: SettingsPanelProps): JSX.Element {
  const preset = detectPreset(options.pattern);
  return (
    <section className="panel">
      <h2>Settings</h2>
      <div className="grid">
        <label>Rename preset<select disabled={disabled} value={preset} onChange={(e) => onChange({ ...options, pattern: applyPatternPreset(e.target.value as RenamePreset, options.pattern) })}>
          <option value="webSafe">Web-safe original names</option>
          <option value="prefixIndex">Prefix + index</option>
          <option value="prefixName">Prefix + original name</option>
          <option value="folderName">Folder + original name</option>
      ...
      <p>Example: "My Cute Animal.png" → "{exampleName(options)}"</p>
