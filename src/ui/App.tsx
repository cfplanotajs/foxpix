...
export default function App(): JSX.Element {
  ...
  return (<main className="app">
    ...
    <section className="right">
      ...
      <section className="panel"><h2>Readiness</h2><p className="hint">{previewRows.length === 0 ? 'Needs preview' : estimatesStale ? 'Needs estimate' : 'Ready'}</p><p className="hint">Source: {mode === 'folder' ? (options.input || 'No source selected') : mode === 'files' ? `${options.filePaths?.length ?? 0} selected files` : 'No source selected'}</p><p className="hint">Global format: {(options.outputFormat ?? 'webp').toUpperCase()} • Mix: {(() => { const mix = computeFormatMix(previewRows); return `WebP ${mix.webp}, AVIF ${mix.avif}, JPEG ${mix.jpeg}, PNG ${mix.png}`; })()}</p>{estimatesStale ? <p className="hint warn">Settings changed. Preview or Estimate Sizes again before processing.</p> : null}</section>
      <section className="panel"><h2>Smart Recommendations</h2><ul>{buildRecommendations({ rows: previewRows, includedMap, outputFormat: normalizeOutputFormat(options.outputFormat), estimatesReady: estimateTotals !== null, estimatesStale }).map((n) => <li key={n} className="hint">{n}</li>)}</ul></section>
