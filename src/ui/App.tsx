...
export default function App(): JSX.Element {
  ...
  return (<main className="app">
    ...
    <section className="right">
      ...
      <section className="panel"><h2>Smart Recommendations</h2><ul>{buildRecommendations({ rows: previewRows, includedMap, outputFormat: normalizeOutputFormat(options.outputFormat), estimatesReady: estimateTotals !== null, estimatesStale, outputFolderStatus, patternWarnings: getPatternWarnings({ pattern: options.pattern, prefix: options.prefix, custom: options.custom }) }).map((n) => <li key={n.text} className="hint">{n.text} {n.action ? <button type="button" className="secondary" onClick={() => { if (n.action?.type === 'run-estimate') { void handleEstimate(); setStatus('Estimate Sizes started.'); } if (n.action?.type === 'select-all') { setIncludedMap(Object.fromEntries(previewRows.map((r) => [r.id, true]))); setStatus('Selected all rows.'); } if (n.action?.type === 'set-pattern' && n.action.pattern) { setOptions((prev) => ({ ...prev, pattern: n.action!.pattern! })); setEstimateTotals(null); setStudioPreview(null); setStatus('Pattern updated. Click Preview again.'); } if (n.action?.type === 'set-review-filter' && n.action.filter) { setActiveReviewFilter(n.action.filter); setStatus(`Showing ${n.action.filter} files.`); } if (n.action?.type === 'set-risky-jpeg-rows-to-webp') { const risky = previewRows.filter((r) => includedMap[r.id] !== false && ((r.error ?? '').includes('JPEG does not support transparency'))); if (risky.length === 0) return setStatus('No risky JPEG rows found.'); setFormatOverrides((prev) => { const next = { ...prev }; risky.forEach((r) => { next[r.id] = 'webp'; }); return next; }); setEstimateTotals(null); setStudioPreview(null); setStatus('Risky JPEG rows switched to WebP. Estimate Sizes again.'); } }}>{n.action.label}</button> : null}</li>)}</ul></section>
