...
import { applyFilesSourceOptions, applyFolderSourceOptions, initializeRowsAfterPreview } from './sourceState.js';
...
function inferStatusTone(message: string): 'info'|'success'|'warning'|'error'|'busy' { const m = message.toLowerCase(); if (m.includes('failed') || m.includes('error')) return 'error'; if (m.includes('processing') || m.includes('estimating') || m.includes('preparing') || m.includes('generating')) return 'busy'; if (m.includes('changed') || m.includes('needs') || m.includes('warning')) return 'warning'; if (m.includes('ready') || m.includes('completed') || m.includes('copied') || m.includes('opened')) return 'success'; return 'info'; }
...
export default function App(): JSX.Element {
  ...
  const onOptionsChange = (next: GuiOptions): void => {
    ...
    if ((next.outputFormat ?? 'webp') !== (options.outputFormat ?? 'webp') && previewRows.length > 0) {
      ...
      setStatus('Settings changed. Estimate Sizes again for updated predictions.');
    }
  ...
  const applyFolderSource = (path: string): void => {
    ...
    setEstimateTotals(null);
    setFormatOverrides({});
    setOptions((prev) => applyFolderSourceOptions(prev, path, outputTouched));
  ...
  const pickFiles = async (): Promise<void> => { if (!bridgeAvailable) return; const picked = await window.foxpix.selectImageFiles(); if (!picked || picked.length === 0) return; setPreviewRows([]); setIncludedMap({}); setThumbnailMap({}); setSelectedRowKey(null); setStudioPreview(null); setEstimateTotals(null); setFormatOverrides({}); setOptions((prev) => applyFilesSourceOptions(prev, picked, outputTouched)); setStatus('Source changed. Click Preview.'); };

  const handlePreview = async (): Promise<void> => { if (bridgeError) return void setStatus(bridgeError); if (validationError) return void setStatus(validationError); setBusy(true); setStatus('Preparing preview...'); try { const result = await window.foxpix.preview({ ...options, output: options.output || undefined, formatOverrides }); setPreviewRows(result.rows); setThumbnailMap({}); setIncludedMap(initializeRowsAfterPreview(result.rows)); setEstimatesStale(true); setOutputFolderStatus(result.outputFolderStatus ?? null); setOptions((prev) => ({ ...prev, output: result.outputFolder })); setStatus(result.total === 0 ? 'No supported files found in this source.' : 'Preview ready.'); } catch (error) { setStatus(`Preview failed: ${toMessage(error)}`); } finally { setBusy(false); } };
  ...
  const onDrop = async (event: React.DragEvent<HTMLDivElement>): Promise<void> => {
    ...
    try {
      ...
      if (resolved.kind === 'folder') {
        ...
        setIncludedMap(initializeRowsAfterPreview(result.rows));
        setThumbnailMap({});
        setSelectedRowKey(null);
        setStudioPreview(null);
        setEstimateTotals(null);
  ...
  return (<main className="app workbench">
    <AppHeader />
    ...
    <section className="right inspector-stack column-scroll">
      <StatusBanner message={bridgeError ?? validationError ?? status} tone={inferStatusTone(bridgeError ?? validationError ?? status)} />
      <section className="panel"><h2>Readiness</h2><p className="hint"><span className="pill">{previewRows.length === 0 ? 'Needs preview' : estimatesStale ? 'Needs estimate' : 'Ready'}</span></p><p className="hint">Source: {mode === 'folder' ? (options.input || 'No source selected') : mode === 'files' ? `${options.filePaths?.length ?? 0} selected files` : 'No source selected'}</p><p className="hint">Global format: {(options.outputFormat ?? 'webp').toUpperCase()} • Mix: {(() => { const mix = computeFormatMix(previewRows); return `WebP ${mix.webp}, AVIF ${mix.avif}, JPEG ${mix.jpeg}, PNG ${mix.png}`; })()}</p><p className="hint">{outputFolderStatus?.status === 'exists' ? 'Output folder exists.' : outputFolderStatus?.status === 'will-create' ? 'Output folder will be created during processing.' : outputFolderStatus?.status === 'not-directory' ? 'Output path is not a folder.' : outputFolderStatus?.status === 'not-accessible' ? 'Output folder cannot be accessed.' : ''}</p>{estimatesStale ? <p className="hint warn">Settings changed. Preview or Estimate Sizes again before processing.</p> : null}</section>
      {mode === 'none' ? <section className='panel'><EmptyState title='Start by adding a folder or image files.' description='FoxPix previews web-safe names, estimates output sizes, and converts assets locally.' steps={['Add source','Preview & tune','Process included assets']} footer='Images stay on this computer.'><button type='button' className='secondary' onClick={() => void pickInput()}>Choose Folder</button><button type='button' className='secondary' onClick={() => void pickFiles()}>Choose Image File(s)</button></EmptyState></section> : null}
      ...
      <PreviewTable rows={previewRows} includedMap={includedMap} thumbnailMap={thumbnailMap} activeFilter={activeReviewFilter} onFilterChange={setActiveReviewFilter} onToggleInclude={(id, included) => { setIncludedMap((prev) => ({ ...prev, [id]: included })); setEstimateTotals(null); setEstimatesStale(true); setStatus('Selection changed. Estimate Sizes again.'); }} onSelectAll={() => { setIncludedMap(Object.fromEntries(previewRows.map((r) => [r.id, true]))); setEstimateTotals(null); setEstimatesStale(true); setStatus('Selection changed. Estimate Sizes again.'); }} onDeselectAll={() => { setIncludedMap(Object.fromEntries(previewRows.map((r) => [r.id, false]))); setEstimateTotals(null); setEstimatesStale(true); setStatus('Selection changed. Estimate Sizes again.'); }} onInvertSelection={() => { setIncludedMap(Object.fromEntries(previewRows.map((r) => [r.id, !(includedMap[r.id] !== false)]))); setEstimateTotals(null); setEstimatesStale(true); setStatus('Selection changed. Estimate Sizes again.'); }} selectedRowKey={selectedRowKey} onSelectRow={(key) => { setSelectedRowKey(key); setStudioPreview(null); }} globalFormat={normalizeOutputFormat(options.outputFormat)} formatOverrides={formatOverrides} onSetFormatOverride={(id, format) => { setFormatOverrides((prev) => { const next = { ...prev }; if (!format) delete next[id]; else next[id] = format; return next; }); setStudioPreview(null); setEstimateTotals(null); setStatus('Format override changed. Estimate Sizes again.'); }} onResetAllOverrides={() => { setFormatOverrides(resetAllOverrides()); setStudioPreview(null); setEstimateTotals(null); setStatus('Format override changed. Estimate Sizes again.'); }} onBulkSetIncludedFormat={(format) => { setFormatOverrides((prev) => applyBulkIncludedOverrides(previewRows, includedMap, prev, format)); setStudioPreview(null); setEstimateTotals(null); setStatus('Format override changed. Estimate Sizes again.'); }} onVisibleRowIdsChange={setVisibleRowIds} />

      <section className="panel preview-studio-panel">
        <div className="section-heading"><h2>Preview Studio</h2><span className="pill">Inspector</span></div>
        {!selectedRowKey ? <EmptyState title='No preview selected' description='Select an image from Preview to inspect Original and Optimized output.' /> : (() => {
